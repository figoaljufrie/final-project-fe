...
export interface TenantBooking {
  ...
  bookingNo: string;
  userId: number;
  status: 'waiting_for_payment' | 'waiting_for_confirmation' | 'confirmed' | 'cancelled' | 'expired' | 'completed' | 'rejected';
  totalAmount: number;
...
export class TenantApprovalService {
  ...
  static async confirmPayment(bookingId: number, data: ConfirmPaymentData) {
    const response = await api.patch(`/tenant/bookings/${bookingId}/confirm`, data);
    return response.data.data || response.data;
  }

  // Reject payment
  static async rejectPayment(bookingId: number, data: RejectPaymentData) {
    const response = await api.patch(`/tenant/bookings/${bookingId}/reject`, data);
    return response.data.data || response.data;
  }

  // Cancel user order
  static async cancelUserOrder(bookingId: number, data: CancelOrderData) {
    const response = await api.patch(`/tenant/bookings/${bookingId}/cancel`, data);
    return response.data.data || response.data;
  }
  ...
  static async sendReminder(bookingId: number, data: SendReminderData) {
    const response = await api.post(`/tenant/bookings/${bookingId}/reminder`, data);
    return response.data.data || response.data;
  }
}
