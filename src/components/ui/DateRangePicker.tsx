  ...
  const handleDateClick = (day: number) => {
    ...
    if (!selectedRange.startDate || (selectedRange.startDate && selectedRange.endDate)) {
      ...
      if (clickedDate < selectedRange.startDate) {
        ...
        const newRange = { startDate: clickedDate, endDate: selectedRange.startDate };
        setSelectedRange(newRange);
        onChange?.(newRange);
      } else {
        const newRange = { startDate: selectedRange.startDate, endDate: clickedDate };
        setSelectedRange(newRange);
        onChange?.(newRange);
      }
      setIsOpen(false);
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <button
          ...
          className={clsx(
            "p-2 text-sm rounded-lg transition-all duration-200 hover:bg-rose-100",
            isSelected && "bg-rose-500 text-white hover:bg-rose-600",
            isInRange && !isSelected && "bg-rose-100 text-rose-700",
            !isSelected && !isInRange && "text-gray-700 hover:text-rose-600"
          )}
        >
          {day}
        </button>
  ...
  return (
    <div className={clsx("relative", className)}>
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2 text-left bg-white border border-gray-300 rounded-lg shadow-sm hover:border-rose-300 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-colors"
      >
        <span className={clsx(
          "text-sm",
          selectedRange.startDate || selectedRange.endDate ? "text-gray-900" : "text-gray-500"
        )}>
          {formatDisplayText()}
        </span>
